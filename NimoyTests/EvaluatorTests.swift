...
final class EvaluatorTests: XCTestCase {
    ...
    private func assertNumber(
        ...
        if case let .number(value, resultUnit, _, _) = result {
    ...
    func testNumberWithUnitNoSpaceInExpression() {
        ...
        XCTAssertNotNil(result)
        if case let .number(value, _, _, _) = result {
    ...
    func testReversePercentageContributesFinalValueToSection() {
        let result = evaluator.evaluate("20% of what is 30")
        XCTAssertEqual(result, .number(150, nil, false, false))

        let sum = evaluator.evaluate("sum")
        XCTAssertEqual(sum, .number(150, nil, false, true))
    }

    // MARK: - Section Reset Behavior

    func testHeadingResetsSectionBeforeSum() {
        _ = evaluator.evaluate("10")
        _ = evaluator.evaluate("# Next section")
        _ = evaluator.evaluate("20")

        let sum = evaluator.evaluate("sum")
        XCTAssertEqual(sum, .number(20, nil, false, true))
    }

    func testSumResetsCurrentSection() {
        _ = evaluator.evaluate("10")
        let firstSum = evaluator.evaluate("sum")
        XCTAssertEqual(firstSum, .number(10, nil, false, true))

        _ = evaluator.evaluate("5")
        let secondSum = evaluator.evaluate("sum")
        XCTAssertEqual(secondSum, .number(5, nil, false, true))
    }
    ...
    func testCurrencyConversionFlag() {
        ...
        if case let .number(_, _, isConversion, _) = result {
