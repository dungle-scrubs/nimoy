...
struct SyncedEditorView: NSViewRepresentable {
    ...
    class SyncedCoordinator: NSObject, NSTextViewDelegate {
        ...
        func applyHighlighting(to textView: NSTextView) {
            ...
            let selectedRanges = textView.selectedRanges
            let attributed = SyntaxHighlighter.highlight(textView.string, font: editorFont, lineHeight: kLineHeight, theme: parent.theme)
            textView.textStorage?.setAttributedString(attributed)
            textView.selectedRanges = selectedRanges
        }
        ...
        func updateLayout() {
            ...
            textView.textContainer?.containerSize = NSSize(width: editorWidth - editorInset, height: CGFloat.greatestFiniteMagnitude)
...
class ResultsNSView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for (index, line) in lines.enumerated() {
            ...
            case .text(let str) where str == "Loading...":
