...
class MLXProvider: LLMProvider {
    ...
    func complete(
        ...
        let tempFile = FileManager.default.temporaryDirectory
            .appendingPathComponent("nimoy_prompt_\(UUID().uuidString).txt")
        ...
        return try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global().async {
                do {
                    ...
                    logger
                        .info(
                            "MLX command completed - exitCode: \(exitCode), stdout length: \(stdout.count), stderr length: \(stderr.count)"
                        )
    ...
    private func shellOutputWithDetails(_ command: String) throws -> (stdout: String, stderr: String, exitCode: Int32) {
        ...
        // Close local write ends so background readers get EOF when the child exits.
...
enum MLXError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .commandFailed(exitCode, stderr):
            "MLX command failed (exit \(exitCode)): \(stderr)"
        case .emptyOutput:
            "MLX returned empty output"
        case let .parseError(rawOutput):
            "Failed to parse MLX output: \(rawOutput)"
        case let .notAvailable(reason):
            "MLX not available: \(reason)"
        }
    }
}
