...
class Evaluator {
    ...
    func evaluate(_ input: String) -> EvaluationResult? {
        ...
        if let reverseResult = tryParseReversePercentage(trimmed) {
            appendResultToSection(reverseResult)
            return reverseResult
        }

        // Remove inline comments for evaluation
        let withoutComment: String = if let commentIndex = trimmed.range(of: "//") {
            String(trimmed[..<commentIndex.lowerBound]).trimmingCharacters(in: .whitespaces)
        } else {
            trimmed
        }
        ...
        if lowercased.hasPrefix("sum ") || lowercased.hasPrefix("total ") {
            ...
            if parts.count >= 3, parts[1] == "in" || parts[1] == "to" {
                ...
                let result = sumInCurrency(targetCurrency)
                resetSection()
                return result
            }
        }
        ...
        if lowercased.hasPrefix("average ") || lowercased.hasPrefix("avg ") {
            ...
            if parts.count >= 3, parts[1] == "in" || parts[1] == "to" {
                let targetCurrency = parts[2]
                let result = averageInCurrency(targetCurrency)
                resetSection()
                return result
            }
        }
        ...
        if words.count == 3, words[1] == "in" || words[1] == "as" || words[1] == "to" {
            ...
            if let value = variables[varName] {
                ...
                if CryptoPriceCache.shared.isCrypto(targetName) {
                    ...
                    if let cryptoAmount = CryptoPriceCache.shared.convertFromUSD(
                        usdAmount: usdAmount,
                        crypto: targetName
                    ) {
    ...
    private func tryParseReversePercentage(_ input: String) -> EvaluationResult? {
        ...
        // Evaluate the value part (could have units) without mutating evaluator state.
        // Reverse-percentage should only contribute the final computed value.
        let savedVariables = variables
        let savedSectionValues = sectionValues
        let savedSectionCurrencyOrder = sectionCurrencyOrder
        let savedInBlockComment = inBlockComment

        defer {
            variables = savedVariables
            sectionValues = savedSectionValues
            sectionCurrencyOrder = savedSectionCurrencyOrder
            inBlockComment = savedInBlockComment
        }

        if let valueResult = evaluate(valueStr) {
            switch valueResult {
            case let .number(value, unit, _, _):
    ...
    private func tryParseAssignment(_ lowercased: String, original: String) -> EvaluationResult? {
        ...
        guard let varName, let expr else { return nil }
    ...
    private func saveResultToVariable(_ varName: String, result: EvaluationResult) {
        if case let .number(value, unit, _, _) = result {
            variables[varName] = Value(value, unit: unit)
        }
    }
    ...
    private func aggregateWithDefaultCurrency(operation: AggregateOperation) -> (Double, Unit?) {
        ...
        if !currencyCounts.isEmpty {
            ...
            let mostCommon = currencyCounts.filter { $0.value == maxCount }.map(\.key)
        ...
        for value in sectionValues {
            if let targetUnit, let sourceUnit = value.unit, sourceUnit.category == .currency {
                let converted = UnitConverter.shared.convert(value.number, from: sourceUnit, to: targetUnit)
                total += converted
            } else {
                total += value.number
    ...
    private func tryParseConversion(_ words: [String], original: String) -> EvaluationResult? {
        ...
        if CryptoPriceCache.shared.isCrypto(targetName) {
            ...
            if let cryptoAmount = CryptoPriceCache.shared.convertFromUSD(usdAmount: usdAmount, crypto: targetName) {
                ...
                return .number(
                    cryptoAmount,
                    Unit(name: targetName, symbol: symbol, category: .currency, factor: 1.0, symbolPosition: .after)
                )
            }
            return .error("Price unavailable")
        }
    ...
    private func stripNaturalLanguage(_ input: String) -> String {
        ...
        result = xPattern.stringByReplacingMatches(
            in: result,
            options: [],
        ...
        let forPattern = try! NSRegularExpression(
            pattern: #"\s+for\s+[a-zA-Z][a-zA-Z\s]*?(?=\s*[+\-*/]|\s*$)"#,
            options: .caseInsensitive
        )
        result = forPattern.stringByReplacingMatches(
            in: result,
            options: [],
            range: NSRange(result.startIndex..., in: result),
            withTemplate: " "
        ...
        let onPattern = try! NSRegularExpression(
            pattern: #"\s+on\s+[a-zA-Z][a-zA-Z\s]*?(?=\s*[+\-*/]|\s*$)"#,
            options: .caseInsensitive
        )
        result = onPattern.stringByReplacingMatches(
            in: result,
            options: [],
            range: NSRange(result.startIndex..., in: result),
            withTemplate: " "
        ...
        let fromPattern = try! NSRegularExpression(
            pattern: #"\s+from\s+[a-zA-Z][a-zA-Z\s]*?(?=\s*[+\-*/]|\s*$)"#,
            options: .caseInsensitive
        )
        result = fromPattern.stringByReplacingMatches(
            in: result,
            options: [],
            range: NSRange(result.startIndex..., in: result),
            withTemplate: " "
        ...
        result = spacePattern.stringByReplacingMatches(
            in: result,
            options: [],
            range: NSRange(result.startIndex..., in: result),
            withTemplate: " "
    ...
    func evaluateNode(_ node: ASTNode) throws -> Value {
        ...
        case let .currency(symbol, value):
            let unit = UnitConverter.shared.currencyUnit(for: symbol)
            if let unit, !sectionCurrencyOrder.contains(unit.name) {
        ...
        case let .asPercentOf(numerator, denominator):
            // $5 as a % of $10 = 50%
        ...
        case let .functionCall(name, arg):
        ...
        case let .functionCall2(name, arg1, arg2):
        ...
        case let .withUnit(expr, unitName):
            let value = try evaluateNode(expr)
            let unit = UnitConverter.shared.unit(named: unitName)
            // Track currency order
            if let unit, unit.category == .currency, !sectionCurrencyOrder.contains(unit.name) {
                sectionCurrencyOrder.append(unit.name)
            }
            return Value(value.number, unit: unit)

        case let .conversion(expr, targetUnitName):
            let value = try evaluateNode(expr)
    ...
    private func appendValueToSection(_ value: Value) {
    ...
    private func appendResultToSection(_ result: EvaluationResult) {
        if case let .number(value, unit, _, _) = result {
            appendValueToSection(Value(value, unit: unit))
        }
    }
